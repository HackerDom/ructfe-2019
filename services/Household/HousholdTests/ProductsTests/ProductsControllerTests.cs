...
namespace HouseholdTests.ProductsTests
{
    ...
    internal class ProductsControllerTests
    {
        ...
        public void SetUp()
        {
            var optionBuilder = new DbContextOptionsBuilder<DataBaseContext>().UseSqlServer(connectionString);

            testDbContext = new DataBaseContext(optionBuilder.Options);
            productsController = new ProductsController(testDbContext);
        }
        ...
        public async Task Should_create_product_and_retrieve_it()
        {
            ...
            {
                Name = "Морковь", Calories = 10, Carbohydrate = 10, Fat = 0, Protein = 0.1
            };

            var createdResult =
                (await productsController.PostProduct(product).ConfigureAwait(false)).Result as CreatedAtActionResult;
