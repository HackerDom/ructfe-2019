...
def get_mail_and_pass():
    return (f'{grs_lower(10)}@{grs_lower(5)}.{grs_lower(2)}', grs(25))
...
async def check_service(request: CheckRequest) -> Verdict:
    ...
    async with Api(request.hostname) as api:
        ...
        status, product = await api.add_product(grs(12))
        if status != 201:
        ...
        status, products_list = await api.add_product_via_xml([{'product': get_random_product(), 'manufacturer': grs(15)} for _ in range(3)])
        if status != 200:
            return Verdict.MUMBLE("Can't add product", f"Failed to add product via XML. Expected status is 200, but return {status} with content {products_list}")
        ...
        products_for_dish = [{"productId": p['id'], "weight": randint(50,100)} for p in sample(products_list['items'], k=randint(1,len(products_list['items'])))]
        ...
        if status != 200 or menu != r_menu:
            return Verdict.MUMBLE("Can't find menu", f"Expected status is 200 with {menu},\n but return {status} with content {r_menu}")
    ...
    async with Api(request.hostname) as api:
        ...
        if status != 200 or r_order != order:
            return Verdict.MUMBLE("Can't get order", f"Expected status 200 with {order},\n but return {status} with content {r_order}")
...
async def put_flag_into_the_service(request: PutRequest) -> Verdict:
    async with Api(request.hostname) as api:
        ...
        status, product = await api.add_product(request.flag)
        if status != 201:
            return Verdict.MUMBLE("Can't add product", f"Expected status is 201, but return {status} with content {product}")
...
async def get_flag_from_the_service(request: GetRequest) -> Verdict:
    ...
    async with Api(request.hostname) as api:        
        ...
        if status != 200:
            return Verdict.MUMBLE("Can't get product", f"Can't get product by id. Expected status is 200 and product is {product_id}, \n but return {status} with {r_product}")
            
        if not r_product['manufacturer'] or r_product['manufacturer'] != request.flag:
            return Verdict.CORRUPT("Can't get flag", f"Flag {request.flag} not equal to {r_product['manufacturer']} from manufacturer for product {product_id}. Creds: {login}:{password} ")
    return Verdict.OK()
...
async def put_sflag_into_the_service(request: PutRequest) -> Verdict:
    async with Api(request.hostname) as api:
        ...
        status, products_list = await api.add_product_via_xml([{'product':get_random_product(), 'manufacturer':grs(15)} for _ in range(randint(2,5))])
        if status != 200:
            return Verdict.MUMBLE("Can't add product", f"Failed to add product via XML. Expected status is 200, but return {status} with content {products_list}")
...
async def get_sflag_from_the_service(request: GetRequest) -> Verdict:
    ...
    async with Api(request.hostname) as api:        
        ...
        if dish['id'] != int(dish_id) or dish['recipe'] != request.flag:
            return Verdict.CORRUPT("Can't get flag", f"Flag {request.flag} not equal to {dish['recipe']} from recipe for dish {dish_id}. Creds: {login}:{password} ")
    return Verdict.OK()
